...
std::vector<Function> FunctionExtractor::Get(const analyser::file::File &file) {
  ...
  while ((start = ast.find(marker, start)) != std::string::npos) {
    ...
    Function func{.filename = file.name,
...
FunctionExtractor::FunctionNameLocation
FunctionExtractor::GetNameLocation(const std::string &function_ast) {
  ...
  std::string coords =
      function_ast.substr(coord_start + 1, coord_end - coord_start - 1);
  ...
  std::string end_coords =
      function_ast.substr(dash + 1, end_bracket - dash - 1);
...
std::string
FunctionExtractor::GetNameFromSource(const std::string &function_ast,
...
FunctionExtractor::FindEnclosingClass(const std::string &ast,
  ...
  while ((class_pos = ast.find(class_marker, class_pos)) != std::string::npos) {
    ...
    std::string coords =
        ast.substr(coord_start + 1, coord_end - coord_start - 1);
    ...
    std::string end_coords = ast.substr(
        second_coord_start + 1, second_coord_end - second_coord_start - 1);
    ...
    if (func_loc.start.line > class_start.line ||
        (func_loc.start.line == class_start.line &&
         func_loc.start.col >= class_start.col)) {
      if (func_loc.start.line < class_end.line ||
          (func_loc.start.line == class_end.line &&
           func_loc.start.col <= class_end.col)) {
        ...
        if (name_start != std::string::npos) {
          ...
          if (id_start != std::string::npos) {
            ...
            std::string id_coords = ast.substr(
                id_coord_start + 1, id_coord_end - id_coord_start - 1);
...
std::string FunctionExtractor::GetClassNameFromSource(
    const ClassInfo &class_info, const std::vector<std::string> &lines) {
